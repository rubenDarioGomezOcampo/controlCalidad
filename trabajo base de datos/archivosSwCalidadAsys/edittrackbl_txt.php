	...
	<body>
		<?php include 'header.php'; ?>
			...
			<?php
				...
				$id_estado = mysqli_real_escape_string($conectar, $_POST['id_estado']);
				$id_mov = mysqli_real_escape_string($conectar, $_POST['id_mov']);

				//Cambiar la fecha a tipo año-mes-dia para mysql
				$fecha = date("Y-m-d", strtotime($fecha));		
				
				$sql = "UPDATE estados_contenedor SET fecha='$fecha',id_movimiento_estado_contenedor='$estado',id_puerto_estado_contenedor='$puerto' where id_estado_contenedor=$id_estado";
				...
				if(!mysqli_error($conectar)){
					echo "<div class='alert alert-success' role='alert'>";
						...
						echo "&nbsp;&nbsp;<b>SE ACTUALIZO EL NUEVO ESTADO DEL CONTENEDOR CON BL ".$bl." CORRECTAMENTE EN LA BASE DE DATOS</b><br><p><br><p>";
						...
						echo " &nbsp;&nbsp;<b>NO SE PUDO ACTUALIZAR EL ESTADO DEL BL $bl</b> <br>";
					...
					if ($id_mov=='7' and $estado!='7'){ //Si está editándose el estado final del contenedor y se cambia por otro estado difernte al final, se reabre el bl.
						$result = "UPDATE bl SET bl.id_estadobl_bl='1' WHERE bl.id_bl='$bl'";
						$resultado= mysqli_query($conectar, $result);
					}
					else if ($estado=='7'){ //Si está editándose el estado final del contenedor se cierra el bl.
